...
class SupplierIncludeExportController extends Controller
{
    public function export(Request $request, $selected_supplier)
    {
        ...
        $fileName = "Export " . strtoupper($selected_supplier) . " TRANSPORT INCLUDE " . date("dmY") . ".IRI";
    ...
    public function createExportFile($selected_supplier,  $selected_customer, $selected_installation, $selected_account_period, $path, $fileName)
    {
        ...
        foreach ($data as $item) {
            $row = $item->loadin_date . "|" . $item->delivery_note . "||" . $item->product_code . "     |" . "BITUME  " . "|BITUMPO     " . "||" . $item->supplier_code . "|" . $item->supplier_code . "|" . "02" . "|U| " . $item->quantity . "| " . "0000000,000" . "| " . $item->buying_price . "| " . "0000000,00" . "|S|" . $item->product_label . "       |||" . "BITUME" . "|Y|60121 ||||" . "\n";
            fwrite($handle, $row);
        }
    }
    ...
    function getData($selected_supplier,  $selected_customer, $selected_installation, $selected_account_period)
    {
        ...
        $query = DB::table('standard_b_l_s')->where('transport_include', '=', '1');
